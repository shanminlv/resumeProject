...
String.prototype.template = function(dom) {
  ...
  if (target.indexOf("lsm-repeat") > -1) {
    var repeatArrString = target.match(
        /(lsm-repeat=)(["]|[']){1}([^\'\"]*)(["]|[']){1}/g
      )[0], //repeat Obj string
    ...
    repeatArr.forEach(function(val) {
      ...
      (function() {
        if (html.indexOf("lsm-inrepeat") > -1) {
          var inrepeat = html.split(
              /[<]{1}[^<]*[l]{1}[s]{1}[m]{1}[-]{1}[i]{1}.*[e]{1}[n]{1}[d]{1}[-]{1}[i]{1}[^>]*[>]{1}/g
            ),
            inrepeatTargetStr = html.match(
              /[<]{1}[^<]*[l]{1}[s]{1}[m]{1}[-]{1}[i]{1}.*[e]{1}[n]{1}[d]{1}[-]{1}[i]{1}[^>]*[>]{1}/g
            )[0],
            inrepeatTargetArr = inrepeatTargetStr.match(
              /(lsm-inrepeat=)(["]|[']){1}([^\'\"]*)(["]|[']){1}/g
            )[0],
            inrepeatObjStr = inrepeatTargetArr.substr(14).slice(0, -1),
            inrepeatArr = inrepeatTargetStr.split(
              /[\[]{2}[a-zA-Z1-9\$._#-]*[\]]{2}/g
            ),
            inrepeatString = "";
